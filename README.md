...
```sh
npm run packages:unlink @tinycld/mail
```
...
```sh
cd ../tinycld
npm run dev              # expo + pocketbase (with IMAP/SMTP servers) with mail linked
npm run test:unit        # includes this package's sanitizer, image-proxy, and domain-verify tests
