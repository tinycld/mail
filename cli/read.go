...
// message is the mail_messages row shape the CLI reads. BodyHTML and
// Attachments hold PocketBase's stored file names — content comes from a
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if o.Format == output.JSON {
				return o.Write(cmd.OutOrStdout(), nil, nil, msgs)
			}
			for i, m := range msgs {
				if i > 0 {
