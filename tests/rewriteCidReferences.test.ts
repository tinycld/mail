...
vi.mock('@tinycld/core/lib/pocketbase', () => ({
    pb: {
        files: {
            getURL: ({ id }: { id: string }, filename: string, queryParams?: { token?: string }) =>
                `http://test.invalid/api/files/coll/${id}/${filename}${queryParams?.token ? `?token=${queryParams.token}` : ''}`,
        },
    },
}))
...
describe('rewriteCidReferences', () => {
    ...
    // mail_messages' viewRule is member-scoped — an unauthenticated file URL
    // 404s in the browser (no Authorization header on <img src>), so the
    // rewritten URL must carry the short-lived file token.
    it('appends the file token as a query param', () => {
        const out = rewriteCidReferences(
            '<img src="cid:foo">',
            COLL,
            REC,
    ...
    it('omits the token query param when no token is available yet', () => {
        const out = rewriteCidReferences(
            '<img src="cid:foo">',
            COLL,
            REC,
    ...
    it('leaves unmatched cid: references alone', () => {
        ...
        const out = rewriteCidReferences(html, COLL, REC, { other: 'other.png' }, TOKEN)
    ...
    it('does not rewrite non-img cid: occurrences', () => {
        ...
        const out = rewriteCidReferences(html, COLL, REC, { foo: 'foo.jpg' }, TOKEN)
        expect(out).toBe(html)
    })
})
