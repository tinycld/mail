...
If the deployment has no mail provider configured, sending fails with a clear error. Configuring one is an administrator task; on a deployment run by a service provider it is already done for you.
